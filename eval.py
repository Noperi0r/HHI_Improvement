...
def evaluation(log_file, use_wandb=False):
    with open(log_file, 'w') as f:
        ...
        for replication in range(replication_times):
            ...
            # Log metrics for current replication to wandb
            # Logs FID, matching score, R-precision for each model (gt, gen, etc.)
            if use_wandb:
                wandb_log = {"replication": replication}
                for key, item in mat_score_dict.items():
                    wandb_log[f"{key}/mm_distance"] = item
                for key, item in R_precision_dict.items():
                    wandb_log[f"{key}/r_precision_top1"] = item[0]
                    wandb_log[f"{key}/r_precision_top2"] = item[1]
                    wandb_log[f"{key}/r_precision_top3"] = item[2]
                for key, item in fid_score_dict.items():
                    wandb_log[f"{key}/fid"] = item
                for key, item in div_score_dict.items():
                    wandb_log[f"{key}/diversity"] = item
                if mm_motion_loaders:
                    for key, item in mm_score_dict.items():
                        wandb_log[f"{key}/multimodality"] = item
                wandb.log(wandb_log, step=replication)
        ...
        # Log summary statistics (mean and confidence interval) to wandb
        if use_wandb:
            summary_log = {}
        ...
        for metric_name, metric_dict in all_metrics.items():
            ...
            for model_name, values in metric_dict.items():
                ...
                if isinstance(mean, np.float64) or isinstance(mean, np.float32):
                    ...
                    # Add to wandb summary log
                    if use_wandb:
                        summary_log[f"summary/{model_name}/{metric_name}/mean"] = float(mean)
                        summary_log[f"summary/{model_name}/{metric_name}/ci"] = float(conf_interval)
                    ...
                    if use_wandb:
                        for i in range(len(mean)):
                            summary_log[f"summary/{model_name}/{metric_name}_top{i+1}/mean"] = float(mean[i])
                            summary_log[f"summary/{model_name}/{metric_name}_top{i+1}/ci"] = float(conf_interval[i])

        # Log all summary statistics at once
        if use_wandb:
...
if __name__ == '__main__':
    ...
    if use_wandb:
        ...
        wandb_api_key = getattr(opt, "wandb_api_key", "a0f95db2a87665862aefe307e54b000739821378")
    ...
    if main_opt.dataset_name == "interhuman":
        main_opt.data_root = '/local_datasets/InterHuman_251205'
        ...
        main_opt.data_root = '/local_datasets/InterX'
    ...
    # Initialize wandb after loading options
    # Note: wandb.login() already called earlier
    if use_wandb:
        model_type = "transformer" if opt.use_trans else "vqvae"
        # Use wandb_name if provided, otherwise use default naming convention
        wandb_run_name = getattr(opt, "wandb_name", f"eval_{model_type}_{opt.name}_{opt.which_epoch}")
        wandb.init(
            ...
            config={
                ...
                "replication_times": opt.replication_times,
                "diversity_times": opt.diversity_times,
                "use_trans": opt.use_trans,
            }
        )
        print(f"Wandb initialized: {opt.wandb_entity}/{opt.wandb_project}/{wandb_run_name}")
            ...
            for time_step in opt.time_steps:
                for topkr in opt.topkr:
                    ...
                    for file in os.listdir(opt.model_dir):
                        ...
                        net, ep = load_vq_model(vq_opt, f"{opt.which_epoch}.tar")
                    ...
                    evaluation(log_file, use_wandb=use_wandb)
    else:
        eval_motion_loaders = {}
        for file in os.listdir(opt.model_dir):
            if opt.which_epoch != "all" and opt.which_epoch not in file:
