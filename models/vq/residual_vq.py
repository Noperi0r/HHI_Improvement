...
from utils.ddp_random import ddp_aware_rand, ddp_aware_randint
...
class ResidualVQ(nn.Module):
    ...
    def forward(self, x, return_all_codes = False, sample_codebook_temp = None, force_dropout_index=-1):
        ...
        # DDP-aware quantize dropout decision
        # Original: random.random() < prob (Python random, GPU-specific)
        # Problem: Each GPU makes different dropout decision
        # Solution: Use ddp_aware_rand() to ensure all GPUs make same decision
        dropout_prob_tensor = ddp_aware_rand((1,), device=device)
        should_quantize_dropout = self.training and dropout_prob_tensor.item() < self.quantize_dropout_prob
        ...
        if should_quantize_dropout:
            # DDP-aware random index selection
            # Original: randrange(low, high) (Python random)
            # Problem: Each GPU selects different dropout starting index
            # Solution: Use ddp_aware_randint() for consistent selection
            start_drop_quantize_index = ddp_aware_randint(
