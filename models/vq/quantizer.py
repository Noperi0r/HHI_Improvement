...
from utils.ddp_random import ddp_aware_rand, ddp_aware_randn
...
def gumbel_noise(t):
    """
    DDP-aware Gumbel noise generation

    Original: noise = torch.zeros_like(t).uniform_(0, 1)
    Problem: In Multi-GPU, t.shape[0] is local batch size (e.g., 42)
             while Single GPU uses full batch size (e.g., 256)
             → Different noise sequences!

    Solution: Use ddp_aware_rand() to generate full batch noise,
              then each GPU extracts its rank-specific portion
    """
    # t.shape = (bs × T, nb_code) where bs is local batch size
    batch_size = t.shape[0]
    other_dims = t.shape[1:]

    # Generate DDP-aware uniform random numbers
    noise = ddp_aware_rand((batch_size,) + other_dims, device=t.device)
...
class QuantizeEMAReset(nn.Module):
    ...
    def _tile(self, x):
        """
        DDP-aware codebook initialization with random noise

        Original: out = out + torch.randn_like(out) * std
        Problem: Multi-GPU generates different noise for same codebook position
        Solution: Use ddp_aware_randn() for consistent noise across GPUs
        """
        ...
        if nb_code_x < self.nb_code:
            ...
            # DDP-aware random noise
            noise = ddp_aware_randn(out.shape, device=out.device)
            out = out + noise * std
        else:
            out = x
        return out
