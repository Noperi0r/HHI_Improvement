...
class RVQTokenizerTrainer:
    def __init__(self, args, vq_model, transformer=None):
        ...
        if args.is_train:
            # Only rank 0 creates logger to avoid duplicate logs
            if self.is_main:
        ...
        # Wandb initialization (only rank 0)
        ...
        if self.use_wandb and self.is_main:
            ...
            # Initialize wandb with experiment name from args.wandb_name or args.name
            ...
            wandb.init(
                ...
                config={
                    ...
                    "depth": args.depth,
                    "commit": args.commit,
                    "loss_explicit": args.loss_explicit,
                    "loss_vel": args.loss_vel,
                    "loss_bn": args.loss_bn,
                    "loss_geo": args.loss_geo,
    ...
    def save(self, file_name, ep, total_it):
        # Extract original model from DDP wrapper
        # DDP wraps model as: DDP(model) -> access via .module
        # Original model keys: 'encoder.weight', 'decoder.weight'
        # DDP model keys: 'module.encoder.weight', 'module.decoder.weight'
        # Solution: Use .module to get original model for saving
        model_to_save = (self.vq_model.module
                        if hasattr(self.vq_model, "module")
                        else self.vq_model)

        state = {
            "vq_model": model_to_save.state_dict(),
    ...
    def train(self, train_loader, val_loader, test_loader, eval_wrapper, plot_eval=None):
        ...
        # Only rank 0 prints to avoid duplicate console output
        if self.is_main:
        ...
        while epoch <= self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                if it % self.opt.log_every == 0:
                    ...
                    # Log training metrics to wandb (only rank 0)
                    # Logs multiple loss components: reconstruction, explicit, velocity, bone length, geodesic, foot contact, commit
                    if self.use_wandb and self.is_main:
            ...
            # Computes average of each loss component over all validation batches
            if self.use_wandb and self.is_main:
                wandb.log({
                    "val/loss": sum(val_loss) / len(val_loss),
                    "val/loss_rec": sum(val_loss_rec) / len(val_loss_rec),
                    "val/loss_explicit": sum(val_loss_explicit) / len(val_loss_explicit),
                    "val/loss_vel": sum(val_loss_vel) / len(val_loss_vel),
                    "val/loss_bn": sum(val_loss_bn) / len(val_loss_bn),
                    "val/loss_geo": sum(val_loss_geo) / len(val_loss_geo),
                    "val/loss_fc": sum(val_loss_fc) / len(val_loss_fc),
