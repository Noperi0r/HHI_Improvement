...
class WordMotionCrossAttnLayer(nn.Module):
    """
    Word-Motion Cross-Attention Layer

    Purpose: Each motion token learns which words to attend to
    Example: "raises right hand" -> right hand joints attend to "right", "hand"
    """
    def __init__(self, d_model, nhead, dropout):
        super(WordMotionCrossAttnLayer, self).__init__()

        # Normalization layers
        self.norm_motion = nn.LayerNorm(d_model, elementwise_affine=False, eps=1e-6)
        self.norm_word = nn.LayerNorm(d_model, elementwise_affine=False, eps=1e-6)

        # Cross-attention: motion queries word embeddings
        self.cross_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout, batch_first=False)
        self.dropout = nn.Dropout(dropout)

    def forward(self, motion_tokens, word_emb, word_mask, shift, scale, gate):
        """
        Args:
            motion_tokens: [N, B, d] - motion tokens (N=375 for 75time x 5joints)
            word_emb: [M, B, d] - word embeddings (M=~10-20 words)
            word_mask: [B, M] - True for valid words, False for padding
            shift, scale, gate: [B, d] - modulation parameters from sentence embedding

        Returns:
            motion_out: [N, B, d] - refined motion tokens
            attn_map: [B, nhead, N, M] - attention weights (for loss)
        """
        # Step 1: Normalize and apply AdaLN modulation
        # modulate = x * (1 + scale) + shift
        motion_mod = modulate(self.norm_motion(motion_tokens), shift, scale)
        word_mod = self.norm_word(word_emb)

        # Step 2: Cross-attention
        # Q: motion_mod [N, B, d] - what each motion token is looking for
        # K: word_mod [M, B, d] - word candidates to choose from
        # V: word_mod [M, B, d] - actual word information
        # key_padding_mask: ~word_mask means True positions are ignored
        attn_out, attn_map = self.cross_attn(
            query=motion_mod,
            key=word_mod,
            value=word_mod,
            key_padding_mask=~word_mask,  # ignore padding positions
            need_weights=True,  # return attention weights for loss
            average_attn_weights=False  # keep per-head attention
        )

        # Step 3: Residual connection with gating
        # gate controls how much word information to add
        motion_out = motion_tokens + gate.unsqueeze(0) * self.dropout(attn_out)

        return motion_out, attn_map
...
class InterMTransformerBlock(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropout, nbp):
        ...
        # Combined stage: word-motion cross + self-attention + FFN
        # Changed from 6 to 9 chunks: +3 for word-motion cross-attention
        self.adaLN_mod_combined = AdaLNModulation(d_model, 9)

        # NEW: Word-Motion Cross-Attention layer (moved to first position)
        self.word_motion_cross = WordMotionCrossAttnLayer(d_model, nhead, dropout)

        self.self_attn = SelfAttnLayer(d_model, nhead, dropout)
        self.ffn_combined = FFN(d_model, dim_feedforward, dropout)

        # Split stage: spatial-temporal + person interaction + FFN
        # Changed back to 14 chunks (removed word-motion from here)
    ...
    def forward(self, src, cond, word_emb=None, word_mask=None, src_key_padding_mask=None):
        """
        Args:
            src: [N, B, d] - motion tokens with separator
            cond: [B, d] - sentence embedding for AdaLN
            word_emb: [M, B, d] - word embeddings (NEW)
            word_mask: [B, M] - valid word mask (NEW)
            src_key_padding_mask: [B, N] - padding mask
        """
        N, B, d = src.shape

        # ========== Combined Stage ==========
        # AdaLN modulation for word-motion cross + self-attention + FFN (9 chunks)
        shift_word, scale_word, gate_word, \
            shift_self, scale_self, gate_self, \
            shift_ffn_c, scale_ffn_c, gate_ffn_c = self.adaLN_mod_combined(cond)

        # Step 1: Word-Motion Cross-Attention (FIRST - align raw motion tokens with words)
        if word_emb is not None:
            src, attn_map_combined = self.word_motion_cross(
                src, word_emb, word_mask,
                shift_word, scale_word, gate_word
            )
            # Store attention map for alignment loss
            self.attn_maps = (attn_map_combined,)
        else:
            self.attn_maps = None

        # Step 2: Self-Attention (global long-range dependencies)
        ...
        # Step 5: Local Interaction Attention (between two persons)
...
class InterMTransformer(nn.Module):
    ...
    def forward(self, src, cond, word_emb=None, word_mask=None, src_key_padding_mask=None):
        """
        Args:
            src: [N, B, d] - motion tokens with separator (N=751 for 375+1+375)
            cond: [B, d] - sentence embedding
            word_emb: [M, B, d] - word embeddings (NEW)
            word_mask: [B, M] - valid word mask (NEW)
            src_key_padding_mask: [B, N] - padding mask
        """
        ...
        for block in self.blocks:
            src = block(src, cond, word_emb, word_mask, src_key_padding_mask=src_key_padding_mask)

        # Remove separator token and concatenate two persons
